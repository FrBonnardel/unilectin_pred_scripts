...
my_file = Path("python_output/protein_info_uniprot.txt")
if not my_file.is_file():
    file = open("python_output/protein_info_uniprot.txt", "w")
    file.close()
...
#PROTEIN INFO DIC
protein_info={}
output_file = open("python_output/protein_info_uniprot.txt", "a")
#WHILE protein_ids
step = 10
        ...
        with open("tmp_gb.gb", "r") as infile:
            ...
            for uniprot in readUniprot(infile):
                try:
                    ...
                    name = uniprot['DE'].split(';')[0].split(' {')[0].replace("RecName: ","").replace("Full=","").replace("SubName: ","")
                    ...
                    if len(tmp) < 4:
                        print(tmp)
                    if len(tmp) > 0:
                        superkingdom = tmp[0].replace(" ","").replace(".","").replace("unclassifiedsequences","unclassified")
                    if len(tmp) > 1:
